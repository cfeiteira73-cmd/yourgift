...
const STATUS: Record<string, { label: string; color: string; bg: string }> = {
  ...
  paid:      { label: 'Pago ✓',    color: '#b8975e',  bg: 'rgba(184,151,94,0.12)'  },
  overdue:   { label: 'Em Atraso', color: 'rgb(239,68,68)',   bg: 'rgba(239,68,68,0.12)'   },
  cancelled: { label: 'Cancelado', color: 'rgba(240,236,228,0.42)', bg: 'rgba(120,130,150,0.08)' },
};
...
export default function ClientBillingPage() {
  ...
  const s = (st: string) => STATUS[st] ?? { label: st, color: 'rgba(240,236,228,0.42)', bg: 'rgba(120,130,150,0.12)' };
      ...
      <div style={{ padding: '1.5rem 2rem 3rem', maxWidth: '860px' }}>
        ...
        {!loading && invoices.length > 0 && (
          <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.04 }}
            ...
            {[
              { label: 'Valor Pendente', value: `€${totalPending.toLocaleString('pt-PT', { minimumFractionDigits: 2 })}`, color: overdueCount > 0 ? 'rgb(245,158,11)' : '#d4b47a', icon: '⏳' },
        ...
        {loading ? (
          ...
          <div style={{ padding: '3rem', textAlign: 'center', color: 'rgba(240,236,228,0.28)' }}>
            ...
            {invoices.length === 0 && (
              <div style={{ fontSize: '0.72rem', color: 'rgba(240,236,228,0.24)' }}>As faturas aparecerão aqui assim que as encomendas forem confirmadas</div>
            )}
          </div>
        ) : (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem' }}>
                        <span style={{ fontSize: '0.85rem', fontWeight: 700, color: 'rgba(240,236,228,0.75)' }}>{inv.ref}</span>
                        {inv.order_ref && <span style={{ fontSize: '0.6rem', color: 'rgba(240,236,228,0.28)', background: 'rgba(240,236,228,0.06)', borderRadius: '4px', padding: '0.1rem 0.35rem' }}>ref. {inv.order_ref}</span>}
                      </div>
                      <div style={{ fontSize: '0.62rem', color: 'rgba(240,236,228,0.28)' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
                      ...
                      {inv.pdf_url ? (
                        <a href={inv.pdf_url} target="_blank" rel="noopener noreferrer"
                          style={{ background: 'rgba(154,124,74,0.10)', border: '1px solid rgba(154,124,74,0.18)', borderRadius: '6px', padding: '0.25rem 0.5rem', fontSize: '0.65rem', color: '#d4b47a', textDecoration: 'none', fontWeight: 600 }}>
                          ↓ PDF
                        </a>
                      ) : (
                        <a href={`mailto:geral@yourgift.pt?subject=Fatura ${inv.ref}`}
                          style={{ background: 'rgba(240,236,228,0.06)', border: '1px solid rgba(240,236,228,0.10)', borderRadius: '6px', padding: '0.25rem 0.5rem', fontSize: '0.65rem', color: 'rgba(240,236,228,0.42)', textDecoration: 'none', fontWeight: 600 }}>
                          Pedir
                        </a>
                      )}
                    </div>
        ...
        {!loading && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.15 }}
            ...
            <div style={{ fontSize: '0.68rem', color: 'rgba(240,236,228,0.28)', lineHeight: 1.5 }}>
              ...
              <a href="mailto:geral@yourgift.pt?subject=Questão sobre faturação" style={{ color: '#d4b47a', textDecoration: 'none', fontWeight: 600 }}>geral@yourgift.pt</a>
            </div>
          </motion.div>
        )}
      </div>
