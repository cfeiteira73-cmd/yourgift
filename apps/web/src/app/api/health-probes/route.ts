...
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({ error: 'Invalid mode' }, { status: 400 });
    ...
    console.error('[health-probes] GET error:', error);
    return NextResponse.json({ error: 'Health probes unavailable' }, { status: 500 });
  }
}
...
export async function POST(req: NextRequest) {
  ...
  return NextResponse.json({ error: 'Unknown action' }, { status: 400 });
    ...
    console.error('[health-probes] POST error:', error);
    return NextResponse.json({ error: 'Health probe action failed' }, { status: 500 });
  }
}
