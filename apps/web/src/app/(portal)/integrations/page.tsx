...
const INTEGRATIONS = [
  {
    ...
    status:'active', lastSync:'há 2 min', color:'#b8975e',
  ...
  },
  {
    ...
    status:'active', lastSync:'há 5 min', color:'#d4b47a',
  ...
  },
  {
    ...
    status:'active', lastSync:'há 8 min', color:'#b8975e',
  ...
  },
  {
    ...
    status:'active', lastSync:'contínuo', color:'#b8975e',
  ...
  },
  {
    ...
    status:'inactive', lastSync:'—', color:'rgba(240,236,228,0.42)',
...
const STATUS_STYLE = {
  active:   { label:'● Ativo',     color:'#b8975e', bg:'rgba(184,151,94,0.10)' },
  inactive: { label:'○ Inativo',   color:'rgba(240,236,228,0.42)',bg:'rgba(120,130,150,0.1)'},
    ...
    <PortalLayout userName={client?.name ?? undefined} companyName={client?.company ?? undefined} tier={client?.tier ?? undefined}>
      <div style={{ padding:'1.5rem 2rem 3rem', maxWidth:'1000px' }}>
        ...
        <motion.div initial={{ opacity:0, y:-10 }} animate={{ opacity:1, y:0 }} style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'1.25rem', flexWrap:'wrap', gap:'0.75rem' }}>
          <div>
            <h1 style={{ fontSize:'1.4rem', fontWeight:800, color:'#f0ece4', letterSpacing:'-0.03em', marginBottom:'0.2rem' }}>Integrações</h1>
            <p style={{ fontSize:'0.78rem', color:'rgba(240,236,228,0.24)' }}>{activeCount} integrações ativas · ecossistema conectado</p>
          </div>
          <span style={{ fontSize:'0.7rem', fontWeight:700, color:'#b8975e', background:'rgba(184,151,94,0.10)', border:'1px solid rgba(184,151,94,0.18)', borderRadius:'9999px', padding:'0.35rem 0.875rem' }}>
            ● {activeCount} Ativas
          </span>
        </motion.div>
        ...
        <motion.div initial={{ opacity:0, y:10 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.05 }}
          ...
          {[
            { label:'Integrações Ativas',  value:`${activeCount}`,                                             color:'#b8975e',   icon:'✅' },
            { label:'Disponíveis',         value:`${INTEGRATIONS.filter(i=>i.status==='inactive').length}`,    color:'rgba(240,236,228,0.42)',   icon:'○'  },
            { label:'Em Breve',            value:`${INTEGRATIONS.filter(i=>i.status==='coming').length}`,      color:'rgb(245,158,11)',    icon:'◌'  },
            { label:'Última Sincronização',value:'há 2 min',                                                   color:'#d4b47a',    icon:'🔄' },
          ].map((s, i) => (
            <motion.div key={s.label} initial={{ opacity:0, y:8 }} animate={{ opacity:1, y:0 }} transition={{ delay:0.08+i*0.05 }}
              ...
              <div style={{ fontSize:'0.62rem', color:'rgba(240,236,228,0.24)', marginTop:'0.2rem' }}>{s.label}</div>
            </motion.div>
          ))}
        </motion.div>
        ...
        <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} transition={{ delay:0.12 }}
          ...
          {CATEGORIES.map(cat => (
            <button type="button" key={cat}  onClick={() => setCatFilter(cat)}
              style={{ padding:'0.35rem 0.75rem', borderRadius:'9999px', fontSize:'0.72rem', fontWeight:catFilter===cat ? 600 : 400, cursor:'pointer', background: catFilter===cat ? 'rgba(154,124,74,0.12)' : 'rgba(240,236,228,0.04)', color: catFilter===cat ? '#d4b47a' : 'rgba(240,236,228,0.42)', border: catFilter===cat ? '1px solid rgba(154,124,74,0.28)' : '1px solid rgba(240,236,228,0.06)', transition:'all 150ms' }}>
              {cat}
            </button>
          ))}
        </motion.div>
          ...
          {filtered.map((integ, i) => {
            ...
            return (
              <motion.div key={integ.id} initial={{ opacity:0, y:14 }} animate={{ opacity:1, y:0 }}
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:'0.75rem' }}>
                  <div style={{ display:'flex', alignItems:'center', gap:'0.5rem' }}>
                    ...
                    <div>
                      ...
                      <span style={{ fontSize:'0.6rem', color:'rgba(240,236,228,0.24)' }}>{integ.category}</span>
                    </div>
                  </div>
                ...
                <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', paddingTop:'0.5rem', borderTop:'1px solid rgba(240,236,228,0.06)' }}>
                  <div style={{ display:'flex', gap:'0.375rem', alignItems:'center' }}>
                    <span style={{ fontSize:'0.6rem', color:'rgba(240,236,228,0.24)' }}>Sinc. {integ.lastSync}</span>
                    {integ.status === 'active' && (
                      <button type="button" onClick={() => testConnection(integ.id)}
                        style={{ fontSize:'0.58rem', fontWeight:600, padding:'0.1rem 0.4rem', borderRadius:'6px', cursor:'pointer', border:'1px solid rgba(154,124,74,0.18)', background:'rgba(154,124,74,0.08)', color: testing[integ.id] === 'ok' ? '#b8975e' : testing[integ.id] === 'fail' ? 'rgb(239,68,68)' : '#d4b47a', transition:'all 200ms' }}>
                  ...
                  {integ.status === 'active' ? (
                    ...
                    <button type="button" style={{ fontSize:'0.65rem', fontWeight:600, color:'#d4b47a', background:'rgba(154,124,74,0.10)', border:'1px solid rgba(154,124,74,0.18)', borderRadius:'7px', padding:'0.2rem 0.5rem', cursor:'pointer' }}>Ativar</button>
                  ) : (
                    <span style={{ fontSize:'0.62rem', color:'rgba(240,236,228,0.24)' }}>Disponível em breve</span>
                  )}
                </div>
              </motion.div>
            );
