...
function Trend({ pct }: { pct: number }) {
  const color = pct >= 0 ? '#b8975e' : 'rgb(239,68,68)';
  return (
...
function BenchmarkGauge({ score }: { score: number }) {
  const color = score >= 75 ? '#b8975e' : score >= 50 ? 'rgb(245,158,11)' : 'rgb(239,68,68)';
    ...
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '0.35rem' }}>
      <div style={{ position: 'relative', width: '72px', height: '72px' }}>
        <svg viewBox="0 0 72 72" style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)' }}>
          <circle cx="36" cy="36" r="30" fill="none" stroke="rgba(240,236,228,0.06)" strokeWidth="6" />
          <motion.circle
        ...
        <div style={{ position: 'absolute', inset: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
          ...
          <div style={{ fontSize: '0.45rem', color: 'rgba(240,236,228,0.24)', textTransform: 'uppercase', letterSpacing: '0.06em' }}>/100</div>
        </div>
      </div>
  ...
  return (
    <PortalLayout>
      <div style={{ padding: '1.5rem', background: '#090907', minHeight: '100vh' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.25rem', gap: '1rem', flexWrap: 'wrap' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: '0.72rem', color: 'rgba(240,236,228,0.24)', marginBottom: '0.375rem' }}>
                YourGift OS · Todos os sistemas · Visão 360°
              </div>
              {kpis?.computed_at && (
                <div style={{ fontSize: '0.62rem', color: 'rgba(240,236,228,0.24)' }}>
          ...
          <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
            <motion.button whileTap={tapScale} onClick={refreshLive} disabled={refreshing}
              style={{ padding: '0.4rem 0.75rem', background: 'rgba(240,236,228,0.06)', border: '1px solid rgba(240,236,228,0.10)', borderRadius: '8px', color: 'rgba(240,236,228,0.42)', fontSize: '0.68rem', fontWeight: 700, cursor: 'pointer' }}>
              {refreshing ? '⏳' : '↻'} Refresh
            </motion.button>
        ...
        {loading ? (
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '60vh', color: 'rgba(240,236,228,0.24)', fontSize: '0.75rem' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
            ...
            {kpis && (
              <>
                {/* Revenue Row */}
                <div>
                  <div style={{ fontSize: '0.6rem', fontWeight: 700, color: 'rgba(240,236,228,0.24)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>💰 RECEITA</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem' }}>
                    {[
                      ...
                      { label: 'Encomendas MTD', value: String(kpis.orders_mtd), sub: `${kpis.orders_7d} últimos 7d`, color: '#d4b47a' },
                      { label: 'Em Processamento', value: String(kpis.orders_in_progress), sub: 'encomendas activas', color: '#b8975e' },
                      { label: 'Orçamentos Pendentes', value: String(kpis.quotes_pending), sub: 'aguardam resposta', color: 'rgb(245,158,11)' },
                    ].map(k => (
                      <motion.div key={k.label} {...fadeUp} style={{ padding: '0.875rem', background: 'rgba(240,236,228,0.04)', borderRadius: '12px', border: '1px solid rgba(240,236,228,0.06)' }}>
                        <div style={{ fontSize: '1.25rem', fontWeight: 900, color: k.color, letterSpacing: '-0.03em', marginBottom: '0.2rem' }}>{k.value}</div>
                        <div style={{ fontSize: '0.6rem', color: 'rgba(240,236,228,0.24)', marginBottom: '0.15rem' }}>{k.label}</div>
                        <div style={{ fontSize: '0.62rem', color: 'rgba(240,236,228,0.42)' }}>{k.sub}</div>
                      </motion.div>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  <div style={{ fontSize: '0.6rem', fontWeight: 700, color: 'rgba(240,236,228,0.24)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>🏭 PROCUREMENT + INVENTÁRIO</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem' }}>
                    {[
                      { label: 'RFQs Ativos', value: String(kpis.rfqs_active), sub: 'em negociação', color: 'rgb(167,139,250)' },
                      { label: 'Poupanças MTD', value: fmtEur(kpis.savings_mtd), sub: 'via procurement AI', color: '#b8975e' },
                      { label: 'Alertas Inventário', value: String(kpis.inventory_alerts), sub: 'stock baixo/esgotado', color: kpis.inventory_alerts > 0 ? 'rgb(245,158,11)' : '#b8975e' },
                ...
                {/* QC + Customer Row */}
                <div>
                  <div style={{ fontSize: '0.6rem', fontWeight: 700, color: 'rgba(240,236,228,0.24)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: '0.5rem' }}>🎯 QUALIDADE + CLIENTES</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem' }}>
                    {[
                      { label: 'Score QC Médio', value: `${kpis.qc_avg_score.toFixed(0)}/100`, sub: `Pass rate ${kpis.qc_pass_rate.toFixed(0)}%`, color: kpis.qc_avg_score >= 75 ? '#b8975e' : 'rgb(245,158,11)' },
                      { label: 'Saúde Clientes', value: `${kpis.avg_customer_health.toFixed(0)}/100`, sub: `${kpis.clients_total} clientes total`, color: kpis.avg_customer_health >= 65 ? '#d4b47a' : 'rgb(245,158,11)' },
                      { label: 'Clientes em Risco', value: String(kpis.at_risk_clients), sub: 'churn risk alto/crítico', color: kpis.at_risk_clients > 0 ? 'rgb(239,68,68)' : '#b8975e' },
                ...
                <motion.div {...fadeUp} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '1rem' }}>
                  ...
                  <div className="yg-card" style={{ padding: '1rem' }}>
                    ...
                    {!snapshot.ai_risks?.length ? (
                      <div style={{ fontSize: '0.68rem', color: 'rgba(240,236,228,0.24)' }}>Nenhum risco identificado.</div>
                    ) : (
                  ...
                  {/* Opportunities */}
                  <div className="yg-card" style={{ padding: '1rem' }}>
                    <h3 style={{ fontSize: '0.7rem', fontWeight: 700, color: '#b8975e', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>📈 Oportunidades</h3>
                  ...
                  {/* Recommended Actions */}
                  <div className="yg-card" style={{ padding: '1rem' }}>
                    <h3 style={{ fontSize: '0.7rem', fontWeight: 700, color: '#d4b47a', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>⚡ Ações Prioritárias</h3>
                      ...
                      snapshot.ai_actions.map((a, i) => (
                        <div key={i} style={{ marginBottom: '0.5rem', padding: '0.5rem 0.625rem', background: 'rgba(77,163,255,0.05)', borderRadius: '8px', border: '1px solid rgba(154,124,74,0.10)' }}>
                          ...
                          <div style={{ display: 'flex', gap: '0.5rem', fontSize: '0.6rem', color: 'rgba(240,236,228,0.24)' }}>
                            ...
                            <span style={{ color: '#b8975e' }}>↑ {a.impact}</span>
                          </div>
                        </div>
                      ))
                    )}
            ...
            <AnimatePresence>
              {forecast && (
                <motion.div {...fadeUp} className="yg-card" style={{ padding: '1.25rem' }}>
                  <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgba(240,236,228,0.24)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '1rem' }}>🔮 Forecast AI — Próximos 30/90 dias</h3>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem', marginBottom: '0.875rem' }}>
                    {[
                      { label: 'Receita 30d', value: fmtEur(Number(forecast.revenue_next_30d ?? 0)), color: '#b8975e' },
                      { label: 'Receita 90d', value: fmtEur(Number(forecast.revenue_next_90d ?? 0)), color: '#d4b47a' },
                      { label: 'Encomendas 30d', value: String(forecast.orders_next_30d ?? '—'), color: 'rgb(167,139,250)' },
                      { label: 'Trajectória', value: String(forecast.growth_trajectory ?? '—'), color: forecast.growth_trajectory === 'accelerating' ? '#b8975e' : forecast.growth_trajectory === 'declining' ? 'rgb(239,68,68)' : 'rgb(245,158,11)' },
                    ].map(k => (
                  ...
                  {Array.isArray(forecast.key_drivers) && (forecast.key_drivers as string[]).length > 0 && (
                    <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                      <span style={{ fontSize: '0.62rem', color: 'rgba(240,236,228,0.24)', fontWeight: 700 }}>Drivers:</span>
                      {(forecast.key_drivers as string[]).map((d, i) => (
                        <span key={i} style={{ fontSize: '0.62rem', padding: '0.15rem 0.5rem', background: 'rgba(184,151,94,0.08)', borderRadius: '5px', color: '#b8975e', border: '1px solid rgba(184,151,94,0.14)' }}>{d}</span>
                      ))}
                    </div>
                  )}
                </motion.div>
            ...
            {!snapshot && !loading && (
              <motion.div {...fadeUp} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '3rem', background: 'rgba(167,139,250,0.04)', borderRadius: '16px', border: '1px dashed rgba(167,139,250,0.2)', gap: '0.875rem' }}>
                ...
                <div style={{ fontSize: '0.88rem', fontWeight: 700, color: 'rgba(240,236,228,0.42)', textAlign: 'center' }}>Sem briefing executivo gerado hoje</div>
                <div style={{ fontSize: '0.72rem', color: 'rgba(240,236,228,0.24)', textAlign: 'center', maxWidth: '320px' }}>
