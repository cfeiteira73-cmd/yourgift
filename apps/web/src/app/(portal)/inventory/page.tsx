...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string; dot: string }> = {
  in_stock:      { label: 'Em Stock',    color: 'rgb(99,230,190)',   bg: 'rgba(99,230,190,0.1)',   dot: '#63e6be' },
  ...
  reserved:      { label: 'Reservado',  color: 'rgb(77,163,255)',   bg: 'rgba(77,163,255,0.1)',   dot: '#4da3ff' },
...
function StockBar({ qty, reorder, capacity = 500 }: { qty: number; reorder: number; capacity?: number }) {
  ...
  const color = qty <= 0 ? 'rgb(239,68,68)' : qty <= reorder ? 'rgb(245,158,11)' : 'rgb(99,230,190)';
  return (
    <div style={{ height: '4px', background: 'rgba(255,255,255,0.06)', borderRadius: '9999px', overflow: 'hidden', position: 'relative' }}>
      <motion.div
  ...
  return (
    <PortalLayout>
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'rgb(7,17,31)' }}>
        ...
        {/* ─── Left: Inventory List ─── */}
        <div style={{ width: '320px', flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255,255,255,0.06)', height: '100%', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '1.125rem 1rem 0.875rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
              <div>
                ...
                <div style={{ fontSize: '0.62rem', color: 'rgb(80,92,110)', marginTop: '0.1rem' }}>
              ...
              <div style={{ display: 'flex', gap: '0.35rem' }}>
                <motion.button whileTap={tapScale} onClick={loadAnalytics}
                  style={{ padding: '0.35rem 0.625rem', background: 'rgba(77,163,255,0.1)', border: '1px solid rgba(77,163,255,0.25)', borderRadius: '8px', color: 'rgb(77,163,255)', fontSize: '0.62rem', fontWeight: 700, cursor: 'pointer' }}>
                  Analytics
                </motion.button>
                <motion.button whileTap={tapScale} onClick={() => { setPanelMode('add'); setSelectedItem(null); setAddError(''); }}
                  style={{ padding: '0.35rem 0.625rem', background: 'rgba(99,230,190,0.1)', border: '1px solid rgba(99,230,190,0.25)', borderRadius: '8px', color: 'rgb(99,230,190)', fontSize: '0.62rem', fontWeight: 700, cursor: 'pointer' }}>
                  + Novo
                </motion.button>
              </div>
            </div>
            ...
            <input
              ...
              style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '9px', padding: '0.5rem 0.75rem', color: 'rgb(200,215,235)', fontSize: '0.75rem', marginBottom: '0.5rem' }}
            />
            ...
            {loading ? (
              [...Array(5)].map((_, i) => (
                <div key={i} style={{ padding: '0.75rem', borderRadius: '10px', background: 'rgba(255,255,255,0.03)', marginBottom: '0.375rem' }}>
                  <div style={{ height: '3px', width: '70%', background: 'rgba(255,255,255,0.06)', borderRadius: '4px', marginBottom: '0.5rem' }} />
                ...
                {filtered.map((item, idx) => (
                  <motion.div
                    ...
                    style={{
                      ...
                      background: selectedItem?.id === item.id ? 'rgba(77,163,255,0.08)' : 'rgba(255,255,255,0.03)',
                      border: `1px solid ${selectedItem?.id === item.id ? 'rgba(77,163,255,0.2)' : 'rgba(255,255,255,0.05)'}`,
                  ...
                  >
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '0.35rem', fontSize: '0.62rem', color: 'rgb(100,112,130)' }}>
            ...
            {panelMode === 'list' && (
              <motion.div key="list" {...fadeUp}>
                {reorderRecs ? (
                      ...
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.625rem' }}>
                        {reorderRecs.map((rec, i) => (
                          <div key={i} style={{ padding: '0.875rem', background: 'rgba(245,158,11,0.05)', borderRadius: '10px', border: '1px solid rgba(245,158,11,0.15)' }}>
                            ...
                            <div style={{ fontSize: '0.7rem', color: 'rgb(140,155,175)' }}>{String(rec.action ?? '')} — Qty: <b style={{ color: 'rgb(200,215,235)' }}>{Number(rec.qty ?? 0)}</b></div>
                            <div style={{ fontSize: '0.65rem', color: 'rgb(80,92,110)', marginTop: '0.2rem' }}>{String(rec.reason ?? '')}</div>
                          </div>
                        ))}
                      </div>
                    )}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', color: 'rgb(80,92,110)', gap: '0.75rem' }}>
                    <div style={{ fontSize: '2rem' }}>📦</div>
                    <div style={{ fontSize: '0.88rem', fontWeight: 700, color: 'rgb(140,155,175)' }}>Selecciona um item para ver detalhes</div>
              ...
              <motion.div key="add" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={springGentle}>
                <div className="yg-card" style={{ padding: '1.5rem', maxWidth: '720px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                    ...
                    <div>
                      <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.3rem' }}>Armazém</div>
                  ...
                  <motion.button whileTap={tapScale} onClick={submitAdd} disabled={actionLoading}
                    style={{ marginTop: '1rem', width: '100%', padding: '0.7rem', background: 'rgba(99,230,190,0.15)', border: '1px solid rgba(99,230,190,0.3)', borderRadius: '10px', color: 'rgb(99,230,190)', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer' }}>
              ...
              <motion.div key={`detail-${selectedItem.id}`} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={springGentle}>
                <div style={{ display: 'flex', gap: '1rem', marginBottom: '1rem', alignItems: 'center', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '0.7rem', color: 'rgb(80,92,110)', marginTop: '0.2rem' }}>SKU: {selectedItem.sku} {selectedItem.supplier_name ? `· ${selectedItem.supplier_name}` : ''}</div>
                  </div>
                  <div style={{ display: 'flex', gap: '0.5rem' }}>
                    <motion.button whileTap={tapScale} onClick={() => setPanelMode('move')}
                      style={{ padding: '0.4rem 0.75rem', background: 'rgba(77,163,255,0.12)', border: '1px solid rgba(77,163,255,0.3)', borderRadius: '8px', color: 'rgb(77,163,255)', fontSize: '0.68rem', fontWeight: 700, cursor: 'pointer' }}>
                      + Movimento
                    </motion.button>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem', marginBottom: '1rem' }}>
                  {[
                    ...
                    { label: 'Disponível', value: `${selectedItem.available_qty} un`, color: 'rgb(99,230,190)' },
                    { label: 'Reservado', value: `${selectedItem.reserved_qty} un`, color: 'rgb(77,163,255)' },
                    ...
                    <div key={k.label} style={{ padding: '0.875rem', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <div style={{ fontSize: '0.6rem', color: 'rgb(80,92,110)', marginTop: '0.15rem' }}>{k.label}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  ...
                  <div className="yg-card" style={{ padding: '1rem' }}>
                    <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Detalhes</h3>
                    {[
                      ...
                      <div key={String(k)} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.35rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                        <span style={{ fontSize: '0.68rem', color: 'rgb(80,92,110)' }}>{k}</span>
                  ...
                  {forecast ? (
                    <div className="yg-card" style={{ padding: '1rem' }}>
                      <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>🔮 Forecast AI</h3>
                      {[
                      ...
                      {forecast.reasoning != null && (
                        <div style={{ marginTop: '0.625rem', padding: '0.5rem', background: 'rgba(167,139,250,0.06)', borderRadius: '8px', fontSize: '0.65rem', color: 'rgb(140,155,175)', lineHeight: 1.5 }}>
                    ...
                    <div className="yg-card" style={{ padding: '1rem', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: '0.5rem' }}>
                      ...
                      <div style={{ fontSize: '0.72rem', color: 'rgb(80,92,110)', textAlign: 'center' }}>Clica em &quot;Forecast AI&quot; para prever a procura</div>
                    </div>
                  )}
                </div>
                ...
                <div className="yg-card" style={{ padding: '1rem', marginTop: '1rem' }}>
                  <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Histórico de Movimentos</h3>
                  {movements.length === 0 ? (
                    <div style={{ color: 'rgb(80,92,110)', fontSize: '0.72rem', textAlign: 'center', padding: '1rem' }}>Sem movimentos registados.</div>
                  ) : (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.375rem' }}>
                      {movements.slice(0, 20).map(m => (
                        <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', padding: '0.5rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                          ...
                          <div style={{ flex: 1 }}>
                            ...
                            {m.notes && <span style={{ fontSize: '0.65rem', color: 'rgb(80,92,110)', marginLeft: '0.35rem' }}>— {m.notes}</span>}
                          </div>
                          <span style={{ fontSize: '0.72rem', fontWeight: 700, color: ['dispatch','transfer','write_off'].includes(m.movement_type) ? 'rgb(239,68,68)' : 'rgb(99,230,190)' }}>
                            {['dispatch','transfer','write_off'].includes(m.movement_type) ? '-' : '+'}{m.quantity} un
                          </span>
                          <span style={{ fontSize: '0.62rem', color: 'rgb(80,92,110)', minWidth: '60px', textAlign: 'right' }}>{fmtDate(m.performed_at)}</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <motion.div key="move" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={springGentle}>
                <div className="yg-card" style={{ padding: '1.5rem', maxWidth: '500px' }}>
                  ...
                  <div style={{ display: 'grid', gap: '0.75rem' }}>
                    <div>
                      <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.3rem' }}>Quantidade *</div>
                    ...
                    <div>
                      <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.3rem' }}>Notas</div>
              ...
              <motion.div key="analytics" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={springGentle}>
                ...
                {!analytics ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '200px' }}>
                    <div style={{ color: 'rgb(80,92,110)', fontSize: '0.75rem' }}>A carregar analytics…</div>
                  </div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
                      {[
                        ...
                        { label: 'Total SKUs', value: String(analytics.total_skus ?? 0), color: 'rgb(77,163,255)' },
                        { label: 'Entradas 30d', value: `${analytics.total_receipts_30d ?? 0} un`, color: 'rgb(99,230,190)' },
                        { label: 'Saídas 30d', value: `${analytics.total_dispatches_30d ?? 0} un`, color: 'rgb(116,231,255)' },
                    ...
                    {!!(analytics.category_breakdown) && Object.keys(analytics.category_breakdown as object).length > 0 && (
                      <div className="yg-card" style={{ padding: '1rem' }}>
                        <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Valor por Categoria</h3>
                        {Object.entries(analytics.category_breakdown as Record<string, number>).sort((a, b) => b[1] - a[1]).map(([cat, val]) => (
                          <div key={cat} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.4rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
