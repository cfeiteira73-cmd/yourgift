...
const STATUS_CFG: Record<string, { label: string; color: string; bg: string }> = {
  pending:     { label: 'Pendente',    color: 'rgb(80,92,110)',   bg: 'rgba(80,92,110,0.12)' },
  in_progress: { label: 'Em Curso',   color: 'rgb(77,163,255)',  bg: 'rgba(77,163,255,0.12)' },
  passed:      { label: 'Aprovado',   color: 'rgb(99,230,190)',  bg: 'rgba(99,230,190,0.12)' },
  ...
  return (
    <PortalLayout>
      <div style={{ display: 'flex', height: '100vh', overflow: 'hidden', background: 'rgb(7,17,31)' }}>
        ...
        <div style={{ width: '300px', flexShrink: 0, display: 'flex', flexDirection: 'column', borderRight: '1px solid rgba(255,255,255,0.06)', height: '100%', overflow: 'hidden' }}>
          <div style={{ padding: '1.125rem 1rem 0.875rem', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.75rem' }}>
              <div>
                <h1 style={{ fontSize: '0.88rem', fontWeight: 800, color: 'rgb(220,232,248)', margin: 0 }}>🔬 QC Inspeções</h1>
                <div style={{ fontSize: '0.62rem', color: 'rgb(80,92,110)', marginTop: '0.1rem' }}>{inspections.length} inspeções</div>
              </div>
              <div style={{ display: 'flex', gap: '0.35rem' }}>
                <motion.button whileTap={tapScale} onClick={loadAnalytics}
                  style={{ padding: '0.35rem 0.5rem', background: 'rgba(77,163,255,0.1)', border: '1px solid rgba(77,163,255,0.25)', borderRadius: '7px', color: 'rgb(77,163,255)', fontSize: '0.62rem', fontWeight: 700, cursor: 'pointer' }}>
                  Analytics
                </motion.button>
                <motion.button whileTap={tapScale} onClick={() => { setPanelMode('create'); setSelected(null); }}
                  style={{ padding: '0.35rem 0.5rem', background: 'rgba(99,230,190,0.1)', border: '1px solid rgba(99,230,190,0.25)', borderRadius: '7px', color: 'rgb(99,230,190)', fontSize: '0.62rem', fontWeight: 700, cursor: 'pointer' }}>
                  + Nova
                </motion.button>
              </div>
            </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '0.5rem' }}>
            {loading ? (
              [...Array(4)].map((_, i) => (
                <div key={i} style={{ padding: '0.75rem', borderRadius: '10px', background: 'rgba(255,255,255,0.03)', marginBottom: '0.375rem' }}>
                  <div style={{ height: '3px', width: '60%', background: 'rgba(255,255,255,0.06)', borderRadius: '4px', marginBottom: '0.5rem' }} />
                ...
                {filtered.map((insp, idx) => (
                  <motion.div
                    ...
                    style={{
                      ...
                      background: selected?.id === insp.id ? 'rgba(77,163,255,0.08)' : 'rgba(255,255,255,0.03)',
                      border: `1px solid ${selected?.id === insp.id ? 'rgba(77,163,255,0.2)' : 'rgba(255,255,255,0.05)'}`,
                  ...
                  >
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '0.25rem' }}>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        {insp.supplier_name && <div style={{ fontSize: '0.6rem', color: 'rgb(80,92,110)' }}>{insp.supplier_name}</div>}
                      </div>
                      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '0.25rem' }}>
                        <StatusBadge status={insp.status} />
                        {insp.overall_score != null && (
                          <span style={{ fontSize: '0.6rem', fontWeight: 800, color: insp.overall_score >= 80 ? 'rgb(99,230,190)' : insp.overall_score >= 60 ? 'rgb(245,158,11)' : 'rgb(239,68,68)' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', minWidth: 0 }}>
          ...
          <AnimatePresence mode="wait">
            ...
            {/* Empty list */}
            {panelMode === 'list' && (
              <motion.div key="list" {...fadeUp} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', height: '60vh', gap: '0.75rem', color: 'rgb(80,92,110)' }}>
                <div style={{ fontSize: '2rem' }}>🔬</div>
                <div style={{ fontSize: '0.88rem', fontWeight: 700, color: 'rgb(140,155,175)' }}>Selecciona uma inspeção</div>
              ...
              <motion.div key="create" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={springGentle}>
                <div className="yg-card" style={{ padding: '1.5rem', maxWidth: '600px' }}>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.75rem' }}>
                    {[
                      ...
                      <div key={f.key} style={{ gridColumn: f.key === 'quantity_inspected' ? '1' : 'auto' }}>
                        <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.3rem' }}>{f.label}</div>
                        <input type={f.type ?? 'text'} value={createForm[f.key as keyof typeof createForm]} onChange={e => setCreate(f.key, e.target.value)} placeholder={f.placeholder}
                          style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '9px', padding: '0.55rem 0.75rem', color: 'rgb(200,215,235)', fontSize: '0.78rem' }} />
                      </div>
                    ))}

                    <div>
                      <div style={{ fontSize: '0.62rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: '0.3rem' }}>Tipo de Referência</div>
                      <select value={createForm.reference_type} onChange={e => setCreate('reference_type', e.target.value)}
                        style={{ width: '100%', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '9px', padding: '0.55rem 0.75rem', color: 'rgb(200,215,235)', fontSize: '0.78rem' }}>
                  ...
                  <motion.button whileTap={tapScale} onClick={submitCreate} disabled={actionLoading || !createForm.reference_id || !createForm.product_name}
                    style={{ marginTop: '1rem', width: '100%', padding: '0.7rem', background: 'rgba(77,163,255,0.15)', border: '1px solid rgba(77,163,255,0.35)', borderRadius: '10px', color: 'rgb(77,163,255)', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer' }}>
              ...
              <motion.div key={`detail-${selected.id}`} initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={springGentle}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '1.25rem', gap: '1rem', flexWrap: 'wrap' }}>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: '0.7rem', color: 'rgb(80,92,110)' }}>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.625rem', marginBottom: '1rem' }}>
                  {[
                    { label: 'Inspecionadas', value: selected.quantity_inspected, color: 'rgb(77,163,255)' },
                    { label: 'Aprovadas', value: selected.quantity_passed, color: 'rgb(99,230,190)' },
                    { label: 'Reprovadas', value: selected.quantity_failed, color: 'rgb(239,68,68)' },
                    { label: 'Pass Rate', value: `${Number(selected.pass_rate ?? 0).toFixed(1)}%`, color: Number(selected.pass_rate ?? 0) >= 95 ? 'rgb(99,230,190)' : Number(selected.pass_rate ?? 0) >= 70 ? 'rgb(245,158,11)' : 'rgb(239,68,68)' },
                  ].map(k => (
                    <div key={k.label} style={{ padding: '0.75rem', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.06)' }}>
                      ...
                      <div style={{ fontSize: '0.58rem', color: 'rgb(80,92,110)', marginTop: '0.1rem' }}>{k.label}</div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                  ...
                  <div className="yg-card" style={{ padding: '1rem' }}>
                    <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>📸 Análise de Imagens (CV)</h3>
                      ...
                      <div style={{ marginBottom: '0.75rem' }}>
                        <div style={{ display: 'flex', gap: '0.375rem' }}>
                          <input value={imageUrl} onChange={e => setImageUrl(e.target.value)} placeholder="URL da imagem do produto…"
                            style={{ flex: 1, background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '8px', padding: '0.45rem 0.625rem', color: 'rgb(200,215,235)', fontSize: '0.72rem' }} />
                    ...
                    {images.length === 0 ? (
                      <div style={{ padding: '1rem', textAlign: 'center', color: 'rgb(80,92,110)', fontSize: '0.68rem' }}>Nenhuma imagem adicionada.</div>
                    ) : (
                      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', maxHeight: '280px', overflowY: 'auto' }}>
                        {images.map(img => (
                          <div key={img.id} style={{ padding: '0.625rem', background: 'rgba(255,255,255,0.03)', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.05)' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.25rem' }}>
                              <span style={{ fontSize: '0.65rem', color: 'rgb(80,92,110)' }}>{img.image_type}</span>
                              {img.ai_score != null && (
                                <span style={{ fontSize: '0.65rem', fontWeight: 800, color: img.ai_score >= 80 ? 'rgb(99,230,190)' : img.ai_score >= 60 ? 'rgb(245,158,11)' : 'rgb(239,68,68)' }}>
                            ...
                            {img.ai_labels && img.ai_labels.length > 0 && (
                              <div style={{ fontSize: '0.58rem', color: 'rgb(80,92,110)', marginTop: '0.1rem' }}>
                  ...
                  {/* Defects */}
                  <div className="yg-card" style={{ padding: '1rem' }}>
                    <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>⚠️ Defeitos Registados</h3>
                      ...
                      <div style={{ marginBottom: '0.75rem', padding: '0.625rem', background: 'rgba(239,68,68,0.05)', borderRadius: '8px', border: '1px solid rgba(239,68,68,0.1)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.35rem', marginBottom: '0.35rem' }}>
                          <select value={defectForm.defect_type} onChange={e => setDefect('defect_type', e.target.value)}
                            style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '7px', padding: '0.4rem', color: 'rgb(200,215,235)', fontSize: '0.65rem' }}>
                        ...
                        <input value={defectForm.description} onChange={e => setDefect('description', e.target.value)} placeholder="Descrição do defeito"
                          style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '7px', padding: '0.4rem 0.5rem', color: 'rgb(200,215,235)', fontSize: '0.65rem', marginBottom: '0.35rem' }} />
                        <div style={{ display: 'flex', gap: '0.35rem' }}>
                          <input type="number" value={defectForm.quantity} onChange={e => setDefect('quantity', e.target.value)} placeholder="Qtd"
                            style={{ width: '60px', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '7px', padding: '0.4rem', color: 'rgb(200,215,235)', fontSize: '0.65rem' }} />
                    ...
                    {defects.length === 0 ? (
                      <div style={{ padding: '0.75rem', textAlign: 'center', color: 'rgb(80,92,110)', fontSize: '0.68rem' }}>Nenhum defeito registado.</div>
                    ) : (
                        ...
                        {defects.map(d => {
                          ...
                          return (
                            <div key={d.id} style={{ padding: '0.5rem 0.625rem', background: sev.bg, borderRadius: '7px', border: `1px solid ${sev.color}22` }}>
                              ...
                              {d.description && <div style={{ fontSize: '0.62rem', color: 'rgb(140,155,175)', marginTop: '0.15rem' }}>{d.description}</div>}
                ...
                {['in_progress', 'pending'].includes(selected.status) && (
                  <div className="yg-card" style={{ padding: '1rem', marginTop: '1rem' }}>
                    <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>✓ Concluir Inspeção</h3>
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.625rem', marginBottom: '0.625rem' }}>
                      <div>
                        <div style={{ fontSize: '0.6rem', color: 'rgb(80,92,110)', marginBottom: '0.25rem' }}>Qtd. Aprovadas</div>
                        <input type="number" value={closeForm.quantity_passed} onChange={e => setCloseForm(f => ({ ...f, quantity_passed: e.target.value }))} placeholder="0"
                          style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '8px', padding: '0.5rem', color: 'rgb(200,215,235)', fontSize: '0.75rem' }} />
                      </div>
                      <div>
                        <div style={{ fontSize: '0.6rem', color: 'rgb(80,92,110)', marginBottom: '0.25rem' }}>Qtd. Reprovadas</div>
                    ...
                    <textarea value={closeForm.inspector_notes} onChange={e => setCloseForm(f => ({ ...f, inspector_notes: e.target.value }))} rows={2}
                      ...
                      style={{ width: '100%', boxSizing: 'border-box', background: 'rgba(255,255,255,0.04)', border: '1px solid rgba(255,255,255,0.09)', borderRadius: '8px', padding: '0.5rem 0.625rem', color: 'rgb(200,215,235)', fontSize: '0.72rem', resize: 'vertical', lineHeight: 1.5, marginBottom: '0.625rem' }} />
                    <motion.button whileTap={tapScale} onClick={submitClose} disabled={actionLoading}
                      style={{ width: '100%', padding: '0.65rem', background: 'rgba(99,230,190,0.15)', border: '1px solid rgba(99,230,190,0.3)', borderRadius: '9px', color: 'rgb(99,230,190)', fontSize: '0.78rem', fontWeight: 700, cursor: 'pointer' }}>
            ...
            {panelMode === 'analytics' && (
              <motion.div key="analytics" initial={{ opacity: 0, x: 16 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -16 }} transition={springGentle}>
                ...
                {!analytics ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '200px', color: 'rgb(80,92,110)', fontSize: '0.75rem' }}>A carregar…</div>
                ) : (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '0.75rem' }}>
                      {[
                        { label: 'Total Inspeções', value: String(analytics.total_inspections ?? 0), color: 'rgb(77,163,255)' },
                        { label: 'Aprovadas', value: String(analytics.passed ?? 0), color: 'rgb(99,230,190)' },
                        ...
                        { label: 'Pendentes', value: String(analytics.pending ?? 0), color: 'rgb(80,92,110)' },
                      ].map(k => (
                        <div key={k.label} style={{ padding: '0.875rem', background: 'rgba(255,255,255,0.03)', borderRadius: '10px', border: '1px solid rgba(255,255,255,0.06)' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1rem' }}>
                      ...
                      {!!(analytics.defect_breakdown) && Object.keys(analytics.defect_breakdown as object).length > 0 && (
                        <div className="yg-card" style={{ padding: '1rem' }}>
                          <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Tipos de Defeito</h3>
                          {Object.entries(analytics.defect_breakdown as Record<string, number>).sort((a,b) => b[1]-a[1]).map(([type, count]) => (
                            <div key={type} style={{ display: 'flex', justifyContent: 'space-between', padding: '0.35rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                      ...
                      {Array.isArray(analytics.supplier_scores) && (analytics.supplier_scores as unknown[]).length > 0 && (
                        <div className="yg-card" style={{ padding: '1rem' }}>
                          <h3 style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(80,92,110)', textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: '0.75rem' }}>Score por Fornecedor</h3>
                          {(analytics.supplier_scores as Array<{ name: string; avg_score: number; inspection_count: number }>).map(s => (
                            <div key={s.name} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '0.4rem 0', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                              <div>
                                <div style={{ fontSize: '0.72rem', fontWeight: 700, color: 'rgb(170,185,205)' }}>{s.name}</div>
                                <div style={{ fontSize: '0.58rem', color: 'rgb(80,92,110)' }}>{s.inspection_count} inspeções</div>
                              </div>
                              <span style={{ fontSize: '0.78rem', fontWeight: 800, color: s.avg_score >= 80 ? 'rgb(99,230,190)' : s.avg_score >= 60 ? 'rgb(245,158,11)' : 'rgb(239,68,68)' }}>
