  ...
  return (
    <div style={{ minHeight: '100vh', background: 'rgb(7,17,31)', paddingTop: '5rem' }}>
      <section style={{ padding: '5rem 1.5rem 4rem', textAlign: 'center', maxWidth: '800px', margin: '0 auto' }}>
        <div style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgba(77,163,255,0.08)', border: '1px solid rgba(77,163,255,0.2)', borderRadius: '100px', padding: '0.375rem 1rem', marginBottom: '1.5rem', fontSize: '0.8rem', color: 'rgb(77,163,255)', fontWeight: 600 }}>
          Branded Merchandise
        </div>
        <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: 'rgb(245,247,251)', letterSpacing: '-0.04em', lineHeight: 1.1, marginBottom: '1.25rem' }}>
          A tua marca em<br /><span style={{ color: 'rgb(77,163,255)' }}>cada produto</span>
        </h1>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/quote" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgb(77,163,255)', color: 'rgb(7,17,31)', padding: '0.875rem 1.75rem', borderRadius: '14px', fontWeight: 700, fontSize: '0.95rem', textDecoration: 'none' }}>
            Pedir Orçamento →
          </Link>
      ...
      <section style={{ padding: '2rem 1.5rem 4rem', maxWidth: '1000px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'rgb(245,247,251)', letterSpacing: '-0.03em', marginBottom: '2rem', textAlign: 'center' }}>Categorias de produto</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '1rem' }}>
          {items.map((item) => (
            <div key={item.label} style={{ background: 'linear-gradient(180deg, rgba(255,255,255,0.07) 0%, rgba(255,255,255,0.02) 100%)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '18px', padding: '1.5rem' }}>
              <div style={{ fontSize: '1.75rem', marginBottom: '0.625rem' }}>{item.emoji}</div>
              <p style={{ fontSize: '0.9rem', fontWeight: 700, color: 'rgb(245,247,251)', marginBottom: '0.25rem' }}>{item.label}</p>
              <p style={{ fontSize: '0.78rem', color: 'rgb(120,130,150)' }}>{item.desc}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section style={{ padding: '2rem 1.5rem 6rem', maxWidth: '760px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'rgb(245,247,251)', letterSpacing: '-0.03em', marginBottom: '2rem', textAlign: 'center' }}>Técnicas de personalização</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {techniques.map((t) => (
            <div key={t.name} style={{ background: 'rgba(77,163,255,0.04)', border: '1px solid rgba(77,163,255,0.1)', borderRadius: '14px', padding: '1.25rem 1.5rem', display: 'flex', gap: '1.25rem', alignItems: 'flex-start' }}>
              <div style={{ width: '8px', height: '8px', borderRadius: '50%', background: 'rgb(77,163,255)', marginTop: '5px', flexShrink: 0 }} />
              <div>
                <p style={{ fontSize: '0.9rem', fontWeight: 700, color: 'rgb(245,247,251)', marginBottom: '0.25rem' }}>{t.name}</p>
                <p style={{ fontSize: '0.85rem', color: 'rgb(120,130,150)', lineHeight: 1.5 }}>{t.detail}</p>
              </div>
            </div>
          ))}
        </div>
