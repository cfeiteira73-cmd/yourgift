    ...
    <div style={{ minHeight: '100vh', background: 'rgb(7,17,31)', paddingTop: '5rem' }}>
      <section style={{ padding: '5rem 1.5rem 4rem', textAlign: 'center', maxWidth: '800px', margin: '0 auto' }}>
        ...
        <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: 'rgb(245,247,251)', letterSpacing: '-0.04em', lineHeight: 1.1, marginBottom: '1.25rem' }}>
          Produção ao destino<br /><span style={{ color: 'rgb(77,163,255)' }}>sem fricção</span>
        </h1>
        ...
        <div style={{ display: 'flex', gap: '1rem', justifyContent: 'center', flexWrap: 'wrap' }}>
          <Link href="/enterprise" style={{ display: 'inline-flex', alignItems: 'center', gap: '0.5rem', background: 'rgb(77,163,255)', color: 'rgb(7,17,31)', padding: '0.875rem 1.75rem', borderRadius: '14px', fontWeight: 700, fontSize: '0.95rem', textDecoration: 'none' }}>
            Falar com a equipa →
          </Link>
      ...
      <section style={{ padding: '2rem 1.5rem 4rem', maxWidth: '880px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'rgb(245,247,251)', letterSpacing: '-0.03em', marginBottom: '2rem', textAlign: 'center' }}>O processo completo</h2>
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          {steps.map((step) => (
            <div key={step.num} style={{ display: 'grid', gridTemplateColumns: '64px 1fr', gap: '1.5rem', alignItems: 'flex-start', background: 'linear-gradient(180deg, rgba(255,255,255,0.06) 0%, rgba(255,255,255,0.02) 100%)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '18px', padding: '1.75rem' }}>
              <div style={{ width: '56px', height: '56px', borderRadius: '14px', background: 'rgba(77,163,255,0.1)', border: '1px solid rgba(77,163,255,0.2)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '1.1rem', fontWeight: 900, color: 'rgb(77,163,255)', flexShrink: 0 }}>
                {step.num}
              </div>
              <div>
                <h3 style={{ fontSize: '1rem', fontWeight: 700, color: 'rgb(245,247,251)', marginBottom: '0.375rem' }}>{step.title}</h3>
      ...
      <section style={{ padding: '2rem 1.5rem 6rem', maxWidth: '760px', margin: '0 auto' }}>
        <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'rgb(245,247,251)', letterSpacing: '-0.03em', marginBottom: '2rem', textAlign: 'center' }}>Porquê externalizar?</h2>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))', gap: '0.75rem', marginBottom: '3rem' }}>
          {benefits.map((b) => (
            <div key={b.text} style={{ display: 'flex', alignItems: 'center', gap: '0.875rem', background: 'rgba(77,163,255,0.04)', border: '1px solid rgba(77,163,255,0.1)', borderRadius: '12px', padding: '1rem 1.25rem' }}>
