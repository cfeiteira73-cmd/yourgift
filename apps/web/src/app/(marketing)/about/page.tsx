    ...
    <div style={{ minHeight: '100vh', background: 'rgb(7,17,31)', paddingTop: '5rem' }}>
      ...
      <section style={{ padding: '5rem 1.5rem 4rem', textAlign: 'center', maxWidth: '800px', margin: '0 auto' }}>
        <h1 style={{ fontSize: 'clamp(2rem, 5vw, 3.5rem)', fontWeight: 900, color: 'rgb(245,247,251)', letterSpacing: '-0.04em', lineHeight: 1.1, marginBottom: '1.25rem' }}>
          ...
          <span style={{ color: 'rgb(77,163,255)' }}>feito com propósito</span>
        </h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '1rem' }}>
          {stats.map((s) => (
            <div key={s.label} style={{
              ...
              <p style={{ fontSize: '2.25rem', fontWeight: 900, color: 'rgb(77,163,255)', letterSpacing: '-0.04em', lineHeight: 1 }}>
                {s.value}
              </p>
              <p style={{ fontSize: '0.8rem', color: 'rgb(120,130,150)', marginTop: '0.5rem', fontWeight: 500 }}>
                {s.label}
              </p>
            </div>
          ))}
      ...
      <section style={{ padding: '4rem 1.5rem', maxWidth: '760px', margin: '0 auto' }}>
        <div style={{
          ...
          <h2 style={{ fontSize: '1.5rem', fontWeight: 800, color: 'rgb(245,247,251)', letterSpacing: '-0.03em', marginBottom: '1rem' }}>
            A nossa missão
          </h2>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '1.25rem' }}>
          {values.map((v) => (
            <div key={v.title} style={{
              ...
              <h3 style={{ fontSize: '0.95rem', fontWeight: 700, color: 'rgb(245,247,251)', marginBottom: '0.5rem' }}>{v.title}</h3>
              <p style={{ fontSize: '0.85rem', color: 'rgb(120,130,150)', lineHeight: 1.6 }}>{v.description}</p>
            </div>
          ))}
        </div>
